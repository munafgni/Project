...
function vp_testimonial_shortcode( $atts = array() ) {
    ...
    if( $atts['include_button'] )
        $output .= '<p><a href="' . home_url( 'service-options/visa-assessment' ) . '" class="button button-outline button-mobile-block">Get a Free Visa Estimate</a></p>';
    $output .= '</div>';
    return $output;
}
...
function vp_footer_email_newsletter_shortcode() {
    return '<form class="footer-email-newsletter" method="GET" action="' . home_url( 'visa-newsletter-subscription' ) . '"><input type="email" name="vp_email" placeholder="Enter your email address" /><button type="submit" class="button">Go</button></form>';
}
