jQuery(function($){
	...
	$('#wpforms-304-field_7 .top-level > input').change(function(){
		$(this).parent().nextUntil('.top-level').toggleClass('visible').find('input').prop('checked', this.checked );
	});
	...
	// ReCaptcha Sizing
	function recaptcha_size() {
		$('.g-recaptcha').each(function(){
			$(this).css('transform', 'scale(' + $(this).parent().width() / 304 + ')');
		});
	}
	recaptcha_size();
